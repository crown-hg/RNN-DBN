...
numtrain = 72*96; %前9个月，3月10日和9月17日是去掉的
numtest = 18*96; %第10月
...
out_size=size(trainlabels,2);
r  = sqrt(6) / sqrt(input_size+out_size);
model=cell(1);
% model{1}.W=0.1*randn(size(traindata,2),hidelayer(1));
% model{1}.b=zeros(1,hidelayer);
% model{1}.Wc=0.1*randn(hidelayer(1),size(trainlabels,2));
model{1}.Wc=rand(input_size,out_size)*2*r-r; %##
model{1}.cc=zeros(1,out_size);
...
[m,cost] = bpFine(model, traindata, trainlabels, topfunc, hidefunc,options,costtype);
...
log=sprintf('%s\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%.4f\t%.4f\t%.2f\t%.2f\t%.2f\t%s',datestr(date),func2str(topfunc),func2str(hidefunc),options.Method,options.maxIter,numperiod,numhide,0,week,day,cost,MRE,MAE,RMSE,time,l);
