function model= dbnFit(X, numhid, y, topfunc, hidefunc,varargin)
...
if(H==1)
    model{H}= rbmFit(X, numhid(end), y, topfunc, hidefunc, varargin{H});
else
    if(numopts>=1) %这里是获得visible层的RBM，如果等于0，我们对RBM的参数全取默认值
        model{1}= rbm(X, numhid(1), hidefunc,varargin{1});
    else
        model{1}= rbm(X, numhid(1), hidefunc);
    end
    %train all other RBM's on top of each other
    for i=2:H-1 %这里是获得除了visible层和top层之外的所有隐层
        if(numopts>=i) %小于i，则表明之后的模型全取默认值
            model{i}=rbm(model{i-1}.top, numhid(i), hidefunc, varargin{i});
        else
            model{i}=rbm(model{i-1}.top, numhid(i), hidefunc);
        end
    end

%the last RBM has access to labels too
    if(numopts>=H)%这里是获得top层的RBM，另外标签是回归层，在top层之上
        model{H}= rbmFit(model{H-1}.top, numhid(end), y, topfunc, hidefunc, varargin{H});
    else
        model{H}= rbmFit(model{H-1}.top, numhid(end), y, topfunc, hidefunc);
    end
end