function [net, s] = rnn_rbm(net_temp, X, s, numhid,hidefunc, varargin)
...
% numdims=d; %输入维度784
% numclasses=u;
% W = net_temp.U; %numhid=100,这是输入参数
% c = zeros(1,numdims);
% b = net_temp.b;
% Wc = net_temp.W;
...
% 		negclasses = hidefunc(nhstates*Wc' + repmat(cc,numcases,1));
...
% net.b= bavg;
% net.W= Wcavg;
% s = hidefunc(X*Wavg + s*Wcavg + repmat(bavg,N,1));
net.U=net_temp.U;
net.b = net_temp.b;
net.W = net_temp.W;
[N,d]=size(X); 
s = hidefunc(X*net.U + s*net.W + repmat(net.b,N,1));
