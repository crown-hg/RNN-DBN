...
for month=5:8
    ...
    numday=size(datamonth{month},3); %每月的天数
    if month==8 % 8月份的最后一个时刻是不能用的，所以干脆直接把最后一天都去掉
    ...
    for w=1:numday
        ...
        for l=0:numlink-1 % l是路段数
            for i=l*daytimesize+1:l*daytimesize+daytimesize
                ...
                if daydata(i,2,w)>=daytimesize-3 
                    if w==numday
                         trainlabels(daydata(i,2,w)+(w-1)*daytimesize,l+1)=daydata(n,3,1);
                    else
                         trainlabels(daydata(i,2,w)+(w-1)*daytimesize,l+1)=daydata(n,3,w+1);
                    end
                else
...
save train567test8k1 train test ps;